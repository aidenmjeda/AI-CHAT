...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  return (
    <div className={`flex flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-10 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      {/* New Chat Button */}
      <button className="flex justify-center items-center w-full py-2 mt-10 text-white bg-gradient-to-r from-[#0B3D0B] to-[#116611] text-sm rounded-md cursor-pointer">
        ...
        {chats
          ...
          .map(chat => (
            ...
            >
              ...
              <img src={assets.bin_icon} className="hidden group-hover:block w-4 cursor-pointer dark:invert" alt="" />
            </div>
          ))}
      </div>
      ...
      <div className="flex items-center justify-between gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md">
        ...
        <label className="relative inline-flex cursor-pointer">
          <input
            ...
            checked={theme === 'dark'}
          />
          <div className="w-9 h-5 bg-gray-400 rounded-full peer-checked:bg-green-800 transition-all relative">
      ...
      <div className="flex items-center gap-3 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer group">
        ...
        {user && <img src={assets.logout_icon} className="h-5 cursor-pointer hidden dark:invert group-hover:block" />}
      </div>
